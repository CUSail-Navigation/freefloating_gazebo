...
if __name__ == '__main__':
    ...
    config_file = '%s/config/%s_pid.yaml' % (
        robot_package, robot_file.split('.')[0])

    # load description
    robot_description = etree.fromstring(check_output(
        ['rosrun', 'xacro', 'xacro', robot_abs_file]))

    # init config dictionary
    pid = {'config': {}}

    # parse joints
    joints = [joint for joint in robot_description.findall(
        'joint') if joint.get('type') != "fixed"]
    if len(joints) != 0:
        pid['config']['joints'] = {}
        ...
        for joint in joints:
            ...
            for controller in ['position', 'velocity']:
                pid[name][controller] = {
                    'p': p_default, 'i': i_default, 'd': d_default}
    ...
    if len(thrusters) != 0:
        ...
        body_axis_candidates = {0: 'x', 1: 'y',
                                2: 'z', 3: 'roll', 4: 'pitch', 5: 'yaw'}
        body_axis = {}
        for thruster in thrusters:
            thruster_map = [float(v)
                            for v in thruster.find('map').text.split(' ')]
        ...
        for x in body_axis.values():
            pid[x] = {}
            for controller in ['position', 'velocity']:
                pid[x][controller] = {'p': p_default,
                                      'i': i_default, 'd': d_default}
