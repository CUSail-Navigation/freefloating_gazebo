...
def abspath_to_roslaunch(filename):
    ...
    for d in range(len(filetree)-1, -1, -1):
        package_path = ''
        try:
            package_path = substitution_args.resolve_args(
                '$(find %s)' % filetree[d])
        except:
            pass
        if package_path != '':
...
def parse_launch_file(launch_file):
    ...
    uwsim_node = [node for sub in ['', 'group/']
                  for node in launch_xml.findall(sub+'node') if node.get('type') == 'uwsim']
...
def extract_xml_value(node, tree):
    ...
    for name in tree:
        ...
        if len(child) == 0:
            rospy.loginfo('Could not find ' + name +
                          ' while exploring ' + v.nodeName)
            sys.exit(1)
...
def insert_header_and_write(xml_data, from_file, filename):
    ...
    header = ['<!-- %s -->' % c for c in
              [' %s ' % ('=' * 90),
               ...
               ' |    This document was autogenerated by scene_to_spawner from %-26s | ' % from_file.split(
                  '/')[-1],
               ' %s ' % ('=' * 90)]]
...
def create_spawner(node, gazebo_model_file):
    ...
    spawn_args = [gazebo_model_file.split(
        '.')[-1] == 'sdf' and 'sdf' or 'urdf']
    ...
    if node.tag == 'vehicle':
        param_node = etree.SubElement(
            group_node, 'param', name='%s/robot_description' % name)
        spawn_args.append('%s -param %s/robot_description' % (name, name))
    else:
        param_node = etree.SubElement(
            group_node, 'param', name='%s_description' % name)
        spawn_args.append('%s -param %s_description' % (name, name))

    param_node.set('command', '$(find xacro)/xacro --inorder %s' %
                   abspath_to_roslaunch(gazebo_model_file))
    ...
    spawn_node.set(
        'args', '-%s -model %s -x %s -y %s -z %s -R %s -P %s -Y %s' % tuple(spawn_args))
    return group_node
...
def process_vehicles(vehicles, datapath):
    ...
    for vehicle in vehicles:
        ...
        gazebo_model_file, datadir = uwsim_to_abspath(uwsim_urdf_file.replace(
            '_uwsim.urdf', '.xacro').replace("urdf/", "xacro/"), datapath)
        if datadir == '':
        ...
        if datadir == '':
            ...
            sys.exit(1)
        uwsim_urdf_file = gazebo_model_file.replace('.urdf', '_uwsim.urdf').replace(
            '.xacro', '_uwsim.urdf').replace("xacro/", "urdf/")
        ...
        if 'xacro' in gazebo_model_file:
            ...
            uwsim_urdf_xml = subprocess.Popen(
                ['rosrun', 'xacro', 'xacro', '--inorder', gazebo_model_file], stdout=subprocess.PIPE)
        ...
        if len(meshes) != 0:
            ...
            for mesh in meshes:
                ...
                mesh_file = substitution_args.resolve_args(
                    mesh.get('filename'))
                # print(mesh_file
                ...
                if '.dae' in mesh_file:
                    mesh_osg, datadir_osg = uwsim_to_abspath(
                        mesh_file.replace('.dae', '.obj'), datapath)
                    if datadir_osg != '':
                        mesh_file, datadir = abspath_to_uwsim(
                            mesh_osg, datapath)
                if '.stl' in mesh_file:
                    mesh_osg, datadir_osg = uwsim_to_abspath(
                        mesh_file.replace('.stl', '.obj'), datapath)
                    if datadir_osg != '':
                        mesh_file, datadir = abspath_to_uwsim(
                            mesh_osg, datapath)
                if datadir == '':
            ...
            # write uwsim urdf
            insert_header_and_write(
                uwsim_urdf_xml, gazebo_model_file, uwsim_urdf_file)
...
def process_objects(objcts, datapath):
    ...
    for obj in scene_xml.findall('object'):
        ...
        for ext in gazebo_mesh_ext:
            gazebo_mesh_file, datadir = uwsim_to_abspath(
                os.path.splitext(uwsim_mesh_file)[0] + ext, datapath)
            if datadir != '':
                break
        ...
        for node in ('visual', 'collision'):
            ...
            object_uri.text = 'file://' + \
                abspath_to_roslaunch(gazebo_mesh_file)
            if node == 'collision':
        ...
        insert_header_and_write(
            object_sdf, gazebo_mesh_file, gazebo_model_file)
